# 🛡️ Cybersecurity Transition Lab & Notes
> **Path to SOC Analyst & Security Engineering**

Welcome to my central repository for my transition into Cybersecurity. This project documents my journey from a **Data Analytics** background into **Cyber Operations**.

---

## 👤 About Me
- **Current Focus:** CompTIA Security+ (SY0-701) & Hands-on SOC Operations.
- **Background:** 2+ years as a Data Analyst (Expertise in SQL, Python, and Anomaly Detection).
- **Location:** Transitioning to Perth, WA 🇦🇺.

---

## 🎯 Certification Roadmap (2026)
- [ ] **CompTIA Security+** (In Progress - Target: June 2026)
- [ ] **Blue Team Level 1 (BTL1)** (Target: August 2026)
- [ ] **Microsoft SC-200** (Target: October 2026)

---

## 🧪 Hands-On Labs
*Proof of practical skill beyond theoretical knowledge.*

| Project | Description | Tech Used |
| :--- | :--- | :--- |
| **[SOC Manager Setup](./Labs/Wazuh-Setup.md)** | Installed and configured a Wazuh Manager on Ubuntu Server. | Wazuh, Ubuntu, VirtualBox |
| **[Log Analysis Script](./Labs/Python-Log-Parser.md)** | Python script to parse .evtx logs for brute-force patterns. | Python, JSON |
| **[TryHackMe SOC Level 1](./Labs/THM-Progress.md)** | Documentation of solved rooms and threat hunting tasks. | Splunk, Wireshark |

---

## 📚 Study Notes (CompTIA Security+)
*Structured notes based on Professor Messer’s curriculum.*

### Domain 1: General Security Concepts
- [x] [1.1 The CIA Triad](./1.1-The-CIA-Triad.md)
- [x] [1.2 Control Categories](./1.2-Security-Control-Categories.md)
- [x] [1.3 Control Types](./1.3-Security-Control-Types.md)
- [x] [1.4 Risk Management Fundamentals](./1.4-Non-repudiation.md)

---

## 🛠️ My Lab Environment
- **Host OS:** Windows 11
- **Hypervisor:** VirtualBox
- **Target Machine:** Windows 10 (Sysmon enabled)
- **Monitoring:** Wazuh Manager (Ubuntu 24.04 LTS)
- **Attacker:** Kali Linux 2026.1

---

## 📈 Activity Stats
![GitHub Activity Graph](https://activity-graph.herokuapp.com/graph?username=MiskaturRahman&theme=dracula)

---

## 📫 Connect with Me
- **LinkedIn:** [linkedin.com/in/miskaturrahman](https://www.linkedin.com/in/miskaturrahman/)
- **Email:** mdrahmanmiskatur@gmail.com
- **Portfolio:** [painleetech.com](https://painleetech.com)

---
*"The best way to predict the future is to secure it."*